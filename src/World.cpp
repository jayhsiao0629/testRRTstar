...
bool Line::LineIntersection(const Line& OtherLine) { //Return if two lines has an intersection
    ...
    if (std::fabs(determinant) < epsilon) { // Lines are parallel or collinear
        // Check if the lines are collinear
        if (std::fabs(lis_a1 * OtherLine.m_p1.m_x + lis_b1 * OtherLine.m_p1.m_y - lis_c1) < epsilon) {
            // Check for overlap
            if (std::max(this->m_p1.m_x, this->m_p2.m_x) >= std::min(OtherLine.m_p1.m_x, OtherLine.m_p2.m_x) &&
                std::max(OtherLine.m_p1.m_x, OtherLine.m_p2.m_x) >= std::min(this->m_p1.m_x, this->m_p2.m_x) &&
                std::max(this->m_p1.m_y, this->m_p2.m_y) >= std::min(OtherLine.m_p1.m_y, OtherLine.m_p2.m_y) &&
                std::max(OtherLine.m_p1.m_y, OtherLine.m_p2.m_y) >= std::min(this->m_p1.m_y, this->m_p2.m_y)) {
                return true;
            }
        }
        return false;
    ...
    else
    {
        ...
        if  ((x_intersect >= std::min(this->m_p1.m_x, this->m_p2.m_x) - epsilon) && 
             (x_intersect <= std::max(this->m_p1.m_x, this->m_p2.m_x) + epsilon) &&
             (y_intersect >= std::min(this->m_p1.m_y, this->m_p2.m_y) - epsilon) && 
             (y_intersect <= std::max(this->m_p1.m_y, this->m_p2.m_y) + epsilon) &&
             (x_intersect >= std::min(OtherLine.m_p1.m_x, OtherLine.m_p2.m_x) - epsilon) && 
             (x_intersect <= std::max(OtherLine.m_p1.m_x, OtherLine.m_p2.m_x) + epsilon) &&
             (y_intersect >= std::min(OtherLine.m_p1.m_y, OtherLine.m_p2.m_y) - epsilon) && 
             (y_intersect <= std::max(OtherLine.m_p1.m_y, OtherLine.m_p2.m_y) + epsilon))
            return true;
...
void World::SetAllObs(Graph &g) {

    for (size_t s = 0; s < this->m_obstacles.size(); s++) { //iterate through all obstacles
        g.DrawRect(this->m_obstacles[s].first.m_x, this->m_obstacles[s].first.m_y, this->m_obstacles[s].second.m_x, this->m_obstacles[s].second.m_y, {123, 123, 123}, true, {255, 123, 123 });
    }
}